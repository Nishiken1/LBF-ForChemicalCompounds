...
def get_false_negatives(X_smiles, X, y, model, batch_size=1024):
    ...
    バッチ推論で偽陰性を抽出する
    Args:
        X_smiles: 元のSMILES文字列。
        X: モデル入力 (X_padded, X_numerical) または X_padded。
        y: ラベル (0または1)。
        model: 学習済みモデル。
        batch_size: 推論時のバッチサイズ。
    Returns:
        list: 偽陰性に該当するSMILES文字列。
    """
    model.eval()
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # 偽陰性をチェック
            for i, (pred_val, label_val) in enumerate(zip(outputs, y_batch)):
                if pred_val < 0.5 and label_val == 1:  # 偽陰性条件
                    false_negatives.append(X_smiles[len(false_negatives)])  # バッチ全体の位置に基づく
...
def main():
    ...
    parser.add_argument('--sample_fraction', type=float, default=1.0, help="Fraction of data to use")
    parser.add_argument('--selected_features', nargs='+', help="List of features to use for model training")
    parser.add_argument('--bidirectional', action='store_true', help="Use bidirectional GRU")
    parser.add_argument('--model_type', type=str, default='UnifiedGRU', choices=['UnifiedGRU', 'MinGRU'],
                        help="Model type to use: 'UnifiedGRU' or 'MinGRU'")
    parser.add_argument('--overwrite', action='store_true', help="Overwrite preprocessed data")
    # ★推論時のバッチサイズを指定したい場合は以下の行を追加
    ...
    project_name = f"4_{args.dataset}_{args.model_type}_h{args.hidden_size}_{fpr_str}_{datetime.now().strftime('%Y%m%d')}"
    ...
    # -- バッチサイズを引数 (--inference_batch_size) から取得して使用 --
    test_false_negatives = get_false_negatives(
        X_test_padded,
        X_test_combined,
        y_test,
        model,
